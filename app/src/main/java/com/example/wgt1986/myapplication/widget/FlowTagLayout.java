...
public class FlowTagLayout extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0, childCount = getChildCount(); i < childCount; i++) {
            ...
            //计算两端间距
            int i1 = (flowWidth - (childWidth + 3) * 4) / 2;
            ...
            //第一个标签时
            if (childLeft == 0) {
                childView.layout(left + i1, top, right + i1, bottom);
                childLeft += (mlp.leftMargin + childWidth + mlp.rightMargin + i1);
                //最后一个标签时
            } else if (childLeft + mlp.leftMargin + childWidth + mlp.rightMargin == flowWidth) {
                childView.layout(left, top, right + i1, bottom);
                childLeft += (mlp.leftMargin + childWidth + mlp.rightMargin + i1);
                //中间标签时
            } else {
